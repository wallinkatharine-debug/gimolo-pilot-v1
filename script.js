...
function wireNav(){
  ["navFilters","navFilters2","navFilters3","navFilters4"].forEach(id=> $(id).addEventListener("click",()=>{beep("click"); openSheet();}));
  ["navSpin","navSpin2","navSpin3","navSpin4"].forEach(id=> $(id).addEventListener("click",()=>{beep("click"); setScreen("screenSpin"); updateMicro();}));
  ["navProfile","navProfile2","navProfile3","navProfile4","profileBtn"].forEach(id=> $(id).addEventListener("click",()=>{beep("click"); alert("Profile is a placeholder in this build.");}));
}
...
function wireMain(){
  ...
  document.querySelectorAll(".face").forEach(btn=>{
    btn.addEventListener("click",()=>{ beep("click"); setTone(btn.dataset.tone); });
  });
